<?php

namespace common\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use common\models\Ustozlar;

/**
 * UstozlarSearch represents the model behind the search form of `common\models\Ustozlar`.
 */
class UstozlarSearch extends Ustozlar
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'kafedra_id', 'user_id'], 'integer'],
            [['ismi', 'familiyasi', 'telefon'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Ustozlar::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'kafedra_id' => $this->kafedra_id,
            'user_id' => $this->user_id,
        ]);

        $query->andFilterWhere(['like', 'ismi', $this->ismi])
            ->andFilterWhere(['like', 'familiyasi', $this->familiyasi])
            ->andFilterWhere(['like', 'telefon', $this->telefon]);

        return $dataProvider;
    }
}
